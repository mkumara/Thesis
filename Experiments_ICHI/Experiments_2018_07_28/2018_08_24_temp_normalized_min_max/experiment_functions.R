...
normalizeData2 <- function(filename) {
  ...
  for (i in seq(1,size[1], 12))
  {
    temp <- matrixData[i:(i+11),];
    temp <- round(temp, 2);
    temp[1:12,1] = round((temp[1:12,1] - min(temp[1:12,1]))/max(temp[1:12,1]), 4);
    temp[1:12,2] = round((temp[1:12,2]-min(temp[1:12,2]))/max(temp[1:12,2]), 4);
  ...
  print(filename);
  print(cor(normalizedMatrix[,1], normalizedMatrix[,2]));
  print(cor(normalizedMatrix[,1], normalizedMatrix[,3]));
  return(normalizedMatrix);
}
